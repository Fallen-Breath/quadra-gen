...
是否在单人游戏的内置服务器（integrated server）中管理维度
...
该维度向客户端广播的世界类型。可用值为 `auto`、`flat` 和 `noise`。
...
是否阻止该象限放置新的世界生成内容（地形方块、结构内容、装饰和世界生成生物等）
...
从低到高依次排列的方块层列表。第一层从该维度的最低可建造高度开始铺设，之后每层紧接上一层

空列表是合法的，将生成完全空旷的超平坦地形，且不会生成原版 Void 预设附带的初始平台
